...
const emptyForm = { name: '', employee_code: '', role_category: 'chef' as RoleCategory }
...
export function StaffListPage() {
  ...
  useEffect(() => {
    ...
    const filtered = allStaff.filter(s => {
      ...
      if (query && !s.name.toLowerCase().includes(query) && !s.employee_code.includes(query)) return false
      return true
    })
  ...
  async function loadStaff() {
    const [staffRes, itemsRes] = await Promise.all([
      supabase.from('staff_members').select('*').eq('is_active', true).eq('branch_id', selectedBranchId).order('employee_code', { ascending: true }),
  ...
  function openEdit(s: StaffMember, e: React.MouseEvent) {
    e.stopPropagation()
    setForm({ name: s.name, employee_code: s.employee_code, role_category: s.role_category })
    setEditing(s); setFormError(''); setShowForm(true)
  }
  function closeForm() { setShowForm(false); setEditing(null) }

  async function save() {
    if (!form.name.trim() || !form.employee_code.trim()) { setFormError('Name and employee code are required'); return }
    ...
    if (editing) {
      ...
      const { error } = await supabase.from('staff_members').insert({
        name: form.name, employee_code: form.employee_code,
        role_category: form.role_category, shift: '1',
  ...
  return (
    <div className="px-4 py-5 space-y-4">
                ...
                <button className="flex items-center gap-3 flex-1 min-w-0 text-left" onClick={() => navigate(`/staff/${s.id}`)}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-slate-400 flex-shrink-0">#{s.employee_code}</span>
                    </div>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-end justify-center" onClick={closeForm}>
